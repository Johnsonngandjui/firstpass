...
// One-click cleanup: strip EVERY Position/Scale/Crop keyframe from the clip
// and park it full-frame, centered, uncropped. The escape hatch when
// experiments have stacked stale motion — no Effect Controls surgery needed.
async function hrResetMotion() {
  hrStatus("");
  const project = await ppro.Project.getActiveProject();
  const sequence = project && await project.getActiveSequence();
  if (!sequence) throw new Error("No active sequence — open your timeline.");
  ...
  const { pos, scale, crop } = await hrMotionParams(clip);
  const rect = await sequence.getFrameSize().catch(() => null);
  const jobs = [{ p: scale, v: 100 }];
  for (const k of Object.keys(crop || {})) jobs.push({ p: crop[k], v: 0 });
  {
    const tpl = await hrCurrentValue(pos).catch(() => null);
    jobs.push({ p: pos, v: hrMakeXY(tpl != null ? tpl : [0, 0],
      rect ? rect.width / 2 : 1920, rect ? rect.height / 2 : 1080) });
  }
  for (const { p, v } of jobs) {
    if (!p) continue;
    try {
      const keys = (await p.getKeyframeListAsTickTimes().catch(() => null)) || [];
      for (const tt of keys) {
        await project.lockedAccess(() => project.executeTransaction((c) => {
          c.addAction(p.createRemoveKeyframeAction(tt));
        }, "FirstPass: reset motion"));
        await sleep(30);
      }
      const kf = await hrMakeKf(p, v);
      await project.lockedAccess(() => project.executeTransaction((c) => {
        try { c.addAction(p.createSetTimeVaryingAction(false)); } catch (_) {}
        try { c.addAction(p.createSetValueAction(kf, true)); }
        catch (_) { c.addAction(p.createSetValueAction(kf)); }
...
async function hrCreateHighlight() {
  ...
  // NOTE: overlays are deliberately screen-locked at creation. Copying the
  // clip's motion onto them (tried once) amplifies whatever stale keyframes
  // the clip carries — one bad experiment and every new overlay inherits it.
...
const hrMrBtn = $("#hr-motionreset");
if (hrMrBtn) hrMrBtn.addEventListener("click", () => withBusy(hrMrBtn, "Clearing…", async () => {
  await hrResetMotion();
  if (hrZfBtn) { hrZfBtn.classList.remove("active"); hrZfBtn.textContent = "Zoom into focus"; }
}));
